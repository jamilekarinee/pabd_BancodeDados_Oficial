...
router.get('/edit/:id', function(req, res) {
  ...
  let cmd = "SELECT * FROM tb_genero WHERE id_genero = ?;";
  db.query(cmd, [id], function(erro, listagem){
  if (erro){
  res.send(erro);
  }
  res.render('generos_add', {resultado: listagem[0]});
  });
  });
...
router.put('/edit/:id', function(req, res ){  
  let id         = req.params.id; 
  let nome      = req.body.nome; 
  let cmd = "UPDATE tb_genero SET nome_genero=? WHERE id_genero=?; ";  
  db.query(cmd, [nome, id], function(erro){
    if(erro){      
      res.send(erro); 
    } 
